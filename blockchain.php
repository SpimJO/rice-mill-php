...
<aside class="sidebar">
  <div>
    ...
    <ul class="menu">
      ...
      <li class="active"><a href="blockchain.php"><i class="fa-solid fa-link"></i> Blockchain Log</a></li>
    </ul>
  </div>
  ...
  <section class="table-section">
    <div class="table-container">
      <table>
        ...
        <tbody>
          <?php
$result = $conn->query("SELECT * FROM blockchain_log ORDER BY id desc");
if ($result && $result->num_rows > 0) {
while($row = $result->fetch_assoc()) {
$prev_full = htmlspecialchars($row['previous_hash']);
$curr_full = htmlspecialchars($row['current_hash']);
$prev_short = substr($prev_full, 0, 6) . "…";
$curr_short = substr($curr_full, 0, 6) . "…";
echo "<tr>
<td>{$row['user_id']}</td>
<td>{$row['action']}</td>
<td>{$row['target_user']}</td>
<td><pre class='data-block'>" . htmlspecialchars($row['data']) . "</pre></td>
<td>{$row['timestamp']}</td>
...
echo "<tr><td colspan='8'>No blockchain records found.</td></tr>";
}
?>
        </tbody>
      </table>
