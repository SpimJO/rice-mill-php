...
/* ============================
   ...
   function addBlockchainLog($conn, $user_id, $action, $target, $data) {
    ...
    // Ensure consistent hashing structure and separator
    $recordString = $user_id . '|' . $action . '|' . $target . '|' . $data . '|' . $timestamp . '|' . $prevHash;
    $currentHash = hash('sha256', $recordString);

    // Insert to blockchain_log
    $stmt = $conn->prepare("
        INSERT INTO blockchain_log (user_id, action, target_user, data, timestamp, previous_hash, current_hash)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("issssss", $user_id, $action, $target, $data, $timestamp, $prevHash, $currentHash);
    $stmt->execute();
}
